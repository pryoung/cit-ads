...
;     Ver.4, 01-Jun-2021, Peter Young
;        Fixed bug in implementation of bib_flag such that it was
...
FOR i=0,n-1 DO BEGIN
 ...
 ;
  ...
  cit_fill_strings,a
 ;
 ; cit_get_ads_entry can shuffle the order of the bibcodes, so I have
 ; to remap bib_flag to the new order
 ;
  IF bib_flag[0] NE -1 THEN BEGIN
     bflag=bib_flag
     na=n_elements(a)
     FOR j=0,na-1 DO BEGIN
        ii=where(a[j].bibcode EQ biblist,nii)
        bflag[j]=bib_flag[ii]
     ENDFOR 
  ENDIF 
 ;
 ...
 ;
  ...
  IF nk NE 0 THEN BEGIN
     cit_instr_make_html_list,a[k],year,bib_flag=bflag,outdir=outdir, link=link, $
