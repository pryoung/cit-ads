...
;     Ver.3, 01-Jun-2021, Peter Young
;        Now implements /update in the case where there are no new
;        bibcodes. 
;-
...
;
; 'blist' is the list of bibcodes that either cite the instrument
; paper or satisfy the abstract search criteria.
;
blist=[blist1,blist2]
k=where(blist NE '',nk)
...
;
; Check if the old bib list (biblist) has entries that the new one
; (blist) doesn't have. This doesn't necessarily indicate a
; problem. For example, some bibcodes may have been manually entered
; because they didn't satisfy the search criteria. 
;
...
;
; 'bcode_save' contains the list of the bibcodes that are not in the
; old list.
;
nb=n_elements(blist)
bcode_save=''
...
IF n_elements(bcode_save) GT 1 THEN BEGIN
 ...
 ;
  IF keyword_set(update) THEN BEGIN
   ...
   ; directory.
   ;
    IF new_file EQ 0 THEN file_move,biblist_file,backup_file
   ;
...
ENDIF ELSE BEGIN
  ...
  ;
   IF keyword_set(update) THEN BEGIN
      ...
      openw,lout,biblist_file,/get_lun
      n=n_elements(biblist)
      FOR i=0,n-1 DO printf,lout,biblist[i]
      free_lun,lout
   ENDIF 
ENDELSE 
