...
;
; The for loop here is just in case the call to ADS fails to
; connect. This happens sometimes if the internet connection is not good.
;
FOR i=0,4 DO BEGIN
  bib=cit_get_ads_bibtex(ads_data.bibcode)
  IF n_elements(bib) GT 1 THEN continue
ENDFOR
IF n_elements(bib) LE 1 THEN return
